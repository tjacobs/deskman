...
// Sysfs trees holding the fan and the thermal zones
static const char* HWMON_ROOT = "/sys/class/hwmon";
static const char* THERMAL_ROOT = "/sys/class/thermal";

// Match devices by name, the hwmon and zone numbers move between boots
...
// Sysfs reads report this when the file is missing or unreadable
static const int FAN_READ_FAILED = -1;

// Sysfs paths, found once on the first check
static string fan_duty_path;
static string fan_rpm_path;
static string temperature_path;
static bool fan_paths_ready = false;

// Run of bad samples, the warnings, and when the last check ran
...
void check_fan() {
    ...
    // Sample the fan and the thermal zone
    last_fan_check = now;
...
static void update_fan_warning() {
    ...
    // Nothing to read on a board with no fan
    ...
    // Expected RPM scales with PWM duty, a low target is not worth judging
...
static void find_fan_paths() {
    // Nothing to walk without the hwmon tree
    error_code error;
    if (!filesystem::exists(HWMON_ROOT, error)) {
        return;
    }

    // Take the duty file from the PWM device and the RPM file from the tach
...
// Warn when CPU thermal stays above the warn point
static void update_temperature_warning() {
    // Find the thermal zone on the first pass
    ...
    // Drop a bad sample, sysfs reports thousandths of a degree
    int temperature_milli = read_hwmon_int(temperature_path);
    if (temperature_milli == FAN_READ_FAILED) {
        return;
    }

    // Clear the warning as soon as it cools off
...
static void find_temperature_path() {
    // Nothing to walk without the thermal tree
    error_code error;
    if (!filesystem::exists(THERMAL_ROOT, error)) {
        return;
    }

    // Take the CPU zone the moment it turns up, remember junction as a backup
    ...
    // Settle for the junction zone, still empty when neither exists
