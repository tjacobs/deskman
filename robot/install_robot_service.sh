...
# Usage: ./install_robot_service.sh             (install, enable)
#        ./install_robot_service.sh --start     (install, enable, start now)
#        ./install_robot_service.sh --uninstall (uninstall)
...
TALK_DIR="$(cd "${PROJECT_DIR}/../talk" && pwd)"
LAUNCHER_SCRIPT="${PROJECT_DIR}/robot_service.sh"
TALK_SCRIPT="${TALK_DIR}/talk.py"
PYTHON_BIN="${TALK_DIR}/.venv/bin/python"

# Print usage help
print_usage() {
    echo "Usage: ./install_robot_service.sh [--start|--uninstall]"
