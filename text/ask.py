...
def ask_model(prompt):
    ...
    for _ in range(MAX_TOOL_ROUNDS):
        ...
        if not tool_calls:
            ...
            if needs_set_reminder(prompt) and not reminder_set_used:
                forced = force_set_reminder(prompt, messages, message, reminder_retry_used)
                if forced is True:
            ...
            if needs_cancel_reminder(prompt) and not reminder_cancel_used:
                forced = force_cancel_reminder(prompt, messages, message, reminder_retry_used)
                if forced is True:
                    reminder_retry_used = True
                    continue
                if forced:
            ...
            if needs_list_reminders(prompt) and not needs_set_reminder(prompt) and not needs_cancel_reminder(prompt):
                ...
                if not reminder_list_used:
                    forced = force_list_reminders(prompt, messages, message, reminder_retry_used)
                    if forced is True:
                        reminder_retry_used = True
                        continue
                    if forced:
        ...
        if needs_set_reminder(prompt) and not reminder_set_used:
            forced = force_set_reminder(prompt, messages, None, True)
            if forced:
                remember_turn(messages, forced)
                return forced
        if needs_cancel_reminder(prompt) and not reminder_cancel_used:
            forced = force_cancel_reminder(prompt, messages, None, True)
            if forced:
                remember_turn(messages, forced)
                return forced
        if needs_list_reminders(prompt) and last_list_reminders_result and not needs_set_reminder(prompt) and not needs_cancel_reminder(prompt):
            remember_turn(messages, last_list_reminders_result)
            return last_list_reminders_result
...
def system_prompt_with_memories():
    ...
    reminder_text = format_reminders_for_prompt()
    if reminder_text:
...
def force_set_reminder(prompt, messages, message, already_retried):
    arguments = parse_set_reminder(prompt)

    # First miss, ask the model again with an explicit reminder order
    if not already_retried:
        print("[ask] missing set_reminder, retrying", flush=True)
        if message is not None:
            messages.append(message)
    ...
    print(f"[ask] forced set_reminder -> {result}", flush=True)
    return result

# Retry cancel_reminder once, then cancel it in Python if still missing
def force_cancel_reminder(prompt, messages, message, already_retried):
    name = parse_cancel_reminder_name(prompt)

    # First miss, ask the model again with an explicit cancel order
    if not already_retried:
        print("[ask] missing cancel_reminder, retrying", flush=True)
        if message is not None:
            messages.append(message)
        if name is None:
    ...
    print(f"[ask] forced cancel_reminder -> {result}", flush=True)
    return result

# Retry list_reminders once, then list them in Python if still missing
def force_list_reminders(prompt, messages, message, already_retried):
    # First miss, ask the model again with an explicit list order
    if not already_retried:
        print("[ask] missing list_reminders, retrying", flush=True)
        if message is not None:
            messages.append(message)
    ...
    print(f"[ask] forced list_reminders -> {result}", flush=True)
    return result
...
def seed_reminders_from_memory():
    with reminders_lock:
        ...
        for name in ("dinner", "bedtime"):
            ...
            added = True
            print(f"[ask] seeded reminder {name} at {format_clock_time(hour, minute)}", flush=True)
        if added:
...
def run_tool(tool_call):
    ...
    # Schedule a daily spoken reminder
    if name == "set_reminder":
        result = run_set_reminder(arguments)
        record_tool(name, arguments, result)
        return result

    # Cancel a daily spoken reminder
    if name == "cancel_reminder":
        result = run_cancel_reminder(arguments)
        record_tool(name, arguments, result)
        return result

    # List daily spoken reminders
    if name == "list_reminders":
        result = run_list_reminders(arguments)
        record_tool(name, arguments, result)
        return result
